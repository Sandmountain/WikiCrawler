...
function validate() {
  ...
  if (
    startArticle.toUpperCase() == goalArticle.toUpperCase() ||
    !startArticle ||
    !goalArticle ||
    valid == 2
  ) {
    if (!startArticle && !goalArticle) {
      ...
      document.getElementById("felMeddelande").innerHTML =
        "<p id='felText'>No starting article</p>";
      $("#felMeddelande").fadeIn("fast");
      $(".centerMe3").css({
        color: "#f24848"
      });
      ...
      document.getElementById("felMeddelande").innerHTML =
        "<p id='felText'>No goal article</p>";
      $("#felMeddelande").fadeIn("fast");
      $(".centerMe3").css({
        color: "#f24848"
      });
      ...
      document.getElementById("felMeddelande").innerHTML =
        "<p id='felText'>Same input twice</p>";
      $("#felMeddelande").fadeIn("fast");
      $(".centerMe3").css({
        color: "#f24848"
      });
    ...
    var startUrl =
      ...
      startArticle;
    var goalUrl =
      "https://en.wikipedia.org/api/rest_v1/page/summary/" + goalArticle;
        ...
        $.ajax({
          ...
          success: function(data, status, xhr) {
            ...
            if (data.title == null) {
              ...
              if (
                goalTitle != goalArticle &&
                description != "" &&
                description.includes("may refer to:")
              ) {
                document.getElementById("shortSummary").innerHTML =
                  textInDiv +
                  "<i><span id='menuTextlittle' style='color: #f24848;'>(Autocorrected input)</span><p id='menuTextlittle'> " +
                  description +
                  "</p></i>";
              } else if (
                description != "" &&
                ...
                document.getElementById("shortSummary").innerHTML =
                  textInDiv +
                  "<i><p id='menuTextlittle'>" +
                  description +
                  "</p></i>";
              } else if (description.includes("may refer to:")) {
                document.getElementById("shortSummary").innerHTML =
                  textInDiv +
                  "<i><p id='menuTextlittle'> Could refer to: " +
                  description +
                  "</p></i>";
              } else {
                document.getElementById("shortSummary").innerHTML =
              ...
              if (valid == 2) {
                ...
                document.getElementById("titleForGoal").innerHTML = goalTitle;
                goalArticle = goalTitle;
                shortSummaryForUi =
                  "<i><p id='menuTextlittle'> " + shortSummary + "</p></i>";
              }
            }
          },
...
function GoGame() {
  ...
  if (counter == -1) document.getElementById("antal").innerHTML = 0 + text;
  else document.getElementById("antal").innerHTML = counter + text;

  document.getElementById("currentArticle").innerHTML =
    thisTitle +
    " <span class='thisTitle'>" +
    startArticle.toUpperCase() +
    "</span>"; //fas fa-info-circle
  document.getElementById("serachedArticle").innerHTML =
    tempText +
    " <span class='serachedItem'>" +
    goalArticle.toUpperCase() +
...
function clearContent() {
  ...
  document.getElementById("serachedArticle").innerHTML =
    "SEARCHING FOR ARTICLE WITH TITLE";
